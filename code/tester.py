...
def test1():
    ...
    assert torch.allclose(relu_1_transformer.equation_ub_weights, torch.tensor([[1, 0], [0, 1 / 6]], dtype=torch.float))
    print(relu_1_transformer.equation_lb_weights)
    assert torch.allclose(relu_1_transformer.equation_lb_weights, torch.tensor([[1, 0], [0, 0.25]], dtype=torch.float))

    assert torch.allclose(relu_1_transformer.equation_ub_bias, torch.tensor([0., 5 / 24]))
    assert torch.allclose(relu_1_transformer.equation_lb_bias, torch.tensor([0., 0.]))
...
def test2():
    ...
    assert torch.allclose(relu_1_transformer.equation_ub_weights,
                          torch.tensor([[1, 1], [1 / 3, -1 / 6]], dtype=torch.float))
    assert torch.allclose(relu_1_transformer.equation_lb_weights,
                          torch.tensor([[1, 1], [1 / 2, -1 / 4]], dtype=torch.float))

    # print(relu_1_transformer.ub_bias)
    assert torch.allclose(relu_1_transformer.equation_ub_bias, torch.tensor([0., 1 / 24]))
    assert torch.allclose(relu_1_transformer.equation_lb_bias, torch.tensor([0., -1 / 4]))
